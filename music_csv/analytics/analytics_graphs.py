...
# Top artistes
artistes = pd.read_csv(STORAGE_DIR + 'top_artists_csv.csv', index_col=0, header=0, names=["artist", "plays"])
...
plt.xlabel("Artiste")
plt.tight_layout()
plt.savefig(STORAGE_DIR + 'top_artists_csv.png')
plt.close()

# Genres par heure
genres_hour = pd.read_csv(STORAGE_DIR + 'genres_by_hour_csv.csv', index_col=0)
...
plt.xlabel("Heure")
plt.tight_layout()
plt.savefig(STORAGE_DIR + 'listens_by_hour_csv.png')
plt.close()
...
plt.tight_layout()
plt.savefig(STORAGE_DIR + 'genres_by_hour_heatmap_csv.png')
plt.close()

# Plateforme x pays (top 10 pays)
country_plat = pd.read_csv(STORAGE_DIR + 'listens_by_country_platform_csv.csv', index_col=0)
...
plt.xlabel("Pays")
plt.tight_layout()
plt.savefig(STORAGE_DIR + 'country_platform_csv.png')
plt.close()
