...
def load_events(storage_dir):
    files = glob(os.path.join(storage_dir, "music_events_*.jsonl"))
    all_events = []
    for file in files:
...
def main(storage_dir):
    ...
    print(top_art)
    top_art.to_csv(os.path.join(storage_dir, "top_artists.csv"))

    print("\nGenres préférés par tranche horaire :")
    genres_hour = genres_by_hour(df)
    print(genres_hour)
    genres_hour.to_csv(os.path.join(storage_dir, "genres_by_hour.csv"))

    print("\nNombre d'écoutes par pays / plateforme :")
    country_plat = listens_by_country_platform(df)
    print(country_plat)
    country_plat.to_csv(os.path.join(storage_dir, "listens_by_country_platform.csv"))
